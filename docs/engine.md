...
`<dataDir>/wal.bin` is an append-only binary file. Every write is recorded here **before** the memtable is updated, then the file is `fsync`'d. This is what gives us crash recovery.
...
Records are sorted by key. The bloom filter lets `Get()` skip the index lookup entirely for keys that aren't in this SSTable. False-positive rate is tuned to about 1% with `m ~ 10n` and `k = 7`. The hash function is double-hashing FNV-1a 64.

## Read path

1. Check memtable. Tombstone returns "not found". Hit returns the value.
2. Walk SSTables newest-to-oldest. For each: check bloom (skip if `mayContain` is false). Binary-search the in-memory index. Read the record at the offset. Tombstone returns "not found". Hit returns the value.
3. Miss in every layer returns "not found".
...
Compaction is **not automatic** in v1. You trigger it via the `Compact SSTables` button in the GUI, or `POST /api/collections/.../compact` for admins. (Auto-compaction is on the v1.x roadmap.)
...
- `after`: exclusive cursor for pagination. Returns keys strictly > this value
